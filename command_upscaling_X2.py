...
def command_upscaler(prompt,model,user_id):
    gc.collect()
    ...
    prompt = prompt + f' -n 2 -s 50 -A k_dpmpp_2 -G 0.6 -U 2 -hires_fix --outdir /home/qkrwnstj/capston_design/invokeai/outputs/{user_id}'
...
def main_process(prompt, user_id):
    process1 = multiprocessing.Process(target=command_upscaler, args=(prompt,'revAnimated_v11',user_id))
    process2 = multiprocessing.Process(target=command_upscaler, args=(prompt,'pastelboys2D_v20',user_id))
    process1.start()
    process2.start()
